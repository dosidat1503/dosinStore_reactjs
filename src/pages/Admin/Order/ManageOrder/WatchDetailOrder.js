...
function WatchDetailOrder({ note, setNote, infoOrderDetail }) {
  const { formatPrice, contentPopup, setContentPopup, openPopup } =
  ...
  const saveNote = (madh, note) => {
    ...
    try {
      ...
      request.post("api/saveNote", data).then((res) => {
        setContentPopup({
          title: "Lưu ghi chú",
  ...
  const renderInfoProduct = infoOrderDetail.data_sanPham_relative_CTDH.map(
    (item, index) => (
  ...
  return (
    <div>
      <Popup contentPopup={contentPopup} confirm={"no use"} />
          ...
          <tbody class="table-group-divider">
            <tr>
              <td data-label="Order-code">
                {infoOrderDetail.data_relative_Donhang.TEN !== null
                  ? infoOrderDetail.data_relative_Donhang.TEN
                  : ""}
              </td>
              <td data-label="Name">
                {infoOrderDetail.data_relative_Donhang.SDT}
              </td>
              <td data-label="Phone-number">
                {infoOrderDetail.data_relative_Donhang.DIACHI
                  ? infoOrderDetail.data_relative_Donhang.DIACHI + ", "
                  : ""}
                {infoOrderDetail.data_relative_Donhang.PHUONG_XA
                  ? infoOrderDetail.data_relative_Donhang.PHUONG_XA + ", "
                  : ""}
                {infoOrderDetail.data_relative_Donhang.QUAN_HUYEN
                  ? infoOrderDetail.data_relative_Donhang.QUAN_HUYEN + ", "
                  : ""}
                {infoOrderDetail.data_relative_Donhang.TINH_TP
                  ? infoOrderDetail.data_relative_Donhang.TINH_TP
      ...
      <div className="div_thongTinGiaoHang" id="thongtin_giao_hang_1">
        ...
        <table class="table">
          ...
          <tbody class="table-group-divider">
            <tr>
              <td data-label="Order-code">
                {infoOrderDetail.data_relative_Donhang.MADH}
              </td>
              <td data-label="Name">
                {infoOrderDetail.data_relative_Donhang.NGAYORDER}
              </td>
              <td data-label="Phone-number">
                {" "}
                {infoOrderDetail.data_relative_Donhang.TRANGTHAI_DONHANG}{" "}
              </td>
              <td data-label="Phone-number">
                {" "}
                {infoOrderDetail.data_relative_Donhang.TRANGTHAI_THANHTOAN}{" "}
              </td>
              <td data-label="Phone-number">
                {" "}
                {infoOrderDetail.data_relative_Donhang.HINHTHUC_THANHTOAN}{" "}
              </td>
              <td data-label="Phone-number">
                {" "}
                {formatPrice(
                  infoOrderDetail.data_relative_Donhang.TONGTIEN_SP
                )}{" "}
              </td>
              <td data-label="Phone-number">
                {" "}
                {formatPrice(
                  infoOrderDetail.data_relative_Donhang.PHIVANCHUYEN
                )}{" "}
              </td>
              <td data-label="Phone-number">
                {" "}
                {formatPrice(
                  infoOrderDetail.data_relative_Donhang.TONGTIENDONHANG
                )}{" "}
              </td>
              <td data-label="Phone-number">
                {" "}
                {infoOrderDetail.data_relative_Donhang.VOUCHERGIAM === 0
                  ? 0
                  : formatPrice(
                      infoOrderDetail.data_relative_Donhang.VOUCHERGIAM
                    )}{" "}
              </td>
            </tr>
          </tbody>
      ...
      <div className="div_thongTinGiaoHang" id="thongtin_giao_hang_1">
        ...
        <table class="table">
          ...
          <tbody class="table-group-divider">{renderInfoProduct}</tbody>
        </table>
      </div>
      <h3>Ghi chú:</h3>
      <textarea
      ...
      <div>
        <button
          className="btn__saveNote"
          onClick={() =>
            handleSaveEditNote(infoOrderDetail.data_relative_Donhang.MADH)
          }
        >
