...
function ManageAccountCustomer() {
  ...
  const [orderStatusPointer, setOrderStatusPointer] = useState(
    nameStatusParam
      ? orderStatus[nameStatusParam]?.nameState
      : orderStatus.khachHang.nameState
  );
  ...
  const getInfoOrderForUsers = (itemInOrderStatus_Array, openingPage) => {
    ...
    if (keySearchParams === null && typeSearchParams === null) {
      request
        .get(`/api/getInfoManageAccountCustomer`, {
          params: queryForGetInfoOrderForUsers,
        })
        .then((res) => {
          ...
          if (res.data.data_thongtin_sanpham.length == 0 && openingPage !== 1) {
            ...
            const startIndexOfOderListHelpDelete =
              itemInOrderStatus_Array.value.indexOfOderListHelpDelete;
              ...
              if (
                ...
                return {
                  ...
                  [itemInOrderStatus_Array.key]: {
                    ...
                    spaceGetDataFromOrderList: [
                      ...orderStatus[itemInOrderStatus_Array.key]
                        .spaceGetDataFromOrderList,
                      {
                        ...
                        startIndex:
                          orderStatus[itemInOrderStatus_Array.key].orderList
                            .length,
                        endIndex:
                          res.data.data_thongtin_sanpham.length +
                          orderStatus[itemInOrderStatus_Array.key].orderList
                            .length,
                      },
                    ],
  ...
  const getQuantityOrderToDevidePage = () => {
    request.get("/api/getQuantityAccountCustomerToDevidePage").then((res) => {
        ...
        orderStatus_Array.forEach((itemStatus) => {
          if (itemStatusFromDB.ROLE === itemStatus.value.nameState) {
            const pageQuantityShow = Math.ceil(
              itemStatusFromDB.SL_MATK / numberOrderEachPage
            );
  ...
  const handleSearch = (infoSearchSendRequest) => {
    Navigate(
      `/admin/manageAccountCustomer?keySearch=${infoSearchSendRequest.keySearchSendRequest}&typeSearch=${infoSearchSendRequest.typeSearchSendRequest}`
    );
  ...
  const renderEachProduct = (orderStatus_Array, item, indexOrder) => {
    ...
    // console.log(index)
    return item.value.orderList.slice(index.start, index.end).map(
      (product, index) =>
